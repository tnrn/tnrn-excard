...
#import "EXOCRVECardInfo.h"

/**
 *	@brief 行驶证识别回调EXOCRCompletedVECardBlock, EXOCRCanceledBlock, EXOCRFailedBlock
 *
 *	@discussion 用来设定异步调用的回调
 */
typedef void (^EXOCRCompletedVECardBlock)(int statusCode, EXOCRVECardInfo *veInfo);
typedef void (^EXOCRCanceledBlock)(int statusCode);
typedef void (^EXOCRFailedBlock)(int statusCode, UIImage *recoImg);

@interface EXOCRVECardRecoManager : NSObject
/**
 *	@brief 获取EXOCRVECardRecoManager实例化对象
 *  @return EXOCRVECardRecoManager对象
 */
 ...
 * @brief 静态图片识别方法
 * @param image - 待识别静态图像
 * @param completedBlock - 识别成功回调，获取识别结果EXOCRVECardInfo对象
 * @param EXOCRFailedBlock - 识别失败回调
 */
-(void)recoVECardFromStillImage:(UIImage *)image
                    OnCompleted:(EXOCRCompletedVECardBlock)completedBlock
 ...
 * @param completedBlock - 识别完成回调，获取识别结果EXOCRVECardInfo对象
 * @param EXOCRCanceledBlock - 识别取消回调
 * @param EXOCRFailedBlock - 识别失败回调
 */
-(void)recoVECardFromStreamOnCompleted:(EXOCRCompletedVECardBlock)completedBlock
...
/**
 * @brief 调用行驶证扫描识别(若默认扫描视图，请使用recoVECardFromStreamOnCompleted:OnCanceled:OnFailed:方法调用识别)
 */
-(void)recoVECardFromStreamByCustomScanView;
@end
